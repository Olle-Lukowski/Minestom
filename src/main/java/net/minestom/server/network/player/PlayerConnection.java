...
public abstract class PlayerConnection {
  ...
  private LoginPluginMessageProcessor loginPluginMessageProcessor =
      new LoginPluginMessageProcessor(this);

  private CompletableFuture<List<SelectKnownPacksPacket.Entry>>
      knownPacksFuture =
          null; // Present only when waiting for a response from the client.

  private final Map<NamespaceID, CompletableFuture<byte @Nullable[]>>
      pendingCookieRequests = new ConcurrentHashMap<>();
  ...
  /**
   * Returns a printable identifier for this connection, will be the player
   * username or the connection remote address.
   *
   ...
   */
  public int getServerPort() { return MinecraftServer.getServer().getPort(); }
  ...
  public PlayerPublicKey playerPublicKey() { return playerPublicKey; }
  ...
  public CompletableFuture<byte @Nullable[]> fetchCookie(@NotNull String key) {
    if (getConnectionState() == ConnectionState.CONFIGURATION &&
        getPlayer() == null) {
      // This is a bit of an unfortunate limitation. The player provider blocks
      // the player read virtual thread waiting for the player provider so a
      // cookie response would never be received and the process would deadlock.
      // We cannot create the player provider without blocking the read thread
      // because the client has already sent the initial settings packet, and we
      // need the Player to process the response. We could store the settings on
      // the connection, but it does not seem worth to get around this case.
      throw new IllegalStateException(
          "Cannot fetch cookie in PlayerProvider, use " +
          "AsyncPlayerPreLoginEvent or AsyncPlayerConfigurationEvent");
    }
  ...
  @ApiStatus.Internal
  public void receiveCookieResponse(@NotNull String key,
                                    byte @Nullable[] data) {
    CompletableFuture<byte[]> future =
        pendingCookieRequests.remove(NamespaceID.from(key));
    if (future != null) {
  ...
  /**
   * Gets the login plugin message processor, only available during the login
   * state.
   */
  @ApiStatus.Internal
  public @NotNull LoginPluginMessageProcessor loginPluginMessageProcessor() {
    return Objects.requireNonNull(this.loginPluginMessageProcessor,
                                  "Login plugin message processor is only " +
                                  "available during the login state.");
  }

  @ApiStatus.Internal
  public @NotNull CompletableFuture<List<SelectKnownPacksPacket.Entry>>
  requestKnownPacks(@NotNull List<SelectKnownPacksPacket.Entry> serverPacks) {
    Check.stateCondition(knownPacksFuture != null,
                         "Known packs already pending");
    sendPacket(new SelectKnownPacksPacket(serverPacks));
    return knownPacksFuture = new CompletableFuture<>();
  }

  @ApiStatus.Internal
  public void receiveKnownPacksResponse(
      @NotNull List<SelectKnownPacksPacket.Entry> clientPacks) {
  ...
  public void transfer(@NotNull String host, int port) {
    EventDispatcher.callCancellable(
        new OutgoingTransferEvent(this.player, host, port),
  ...
  public boolean wasTransferred() {
    if (this.wasTransferred == null) {
      throw new IllegalStateException(
          "The player has not completed the handshake process yet!");
    }
