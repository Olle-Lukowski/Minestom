...
public final class HandshakeListener {
  ...
  private final static Logger LOGGER =
      LoggerFactory.getLogger(HandshakeListener.class);

  /**
   * Text sent if a player tries to connect with an invalid version of the
   * client
   */
  private static final Component INVALID_VERSION_TEXT = Component.text(
      "Invalid Version, please use " + MinecraftServer.VERSION_NAME,
      NamedTextColor.RED);

  /**
   * Indicates that a BungeeGuard authentication was invalid due to missing,
  ...
  public static void listener(@NotNull ClientHandshakePacket packet,
    ...
    socketConnection.refreshServerInformation(
        packet.serverAddress(), packet.serverPort(), packet.protocolVersion());
  }

  private static void handlePlayerLogin(@NotNull ClientHandshakePacket packet,
                                        @NotNull PlayerConnection connection) {
    ...
    if (BungeeCordProxy.isEnabled() &&
        connection instanceof PlayerSocketConnection socketConnection) {
      ...
      if (split.length == 3 || split.length == 4) {
        ...
        if (hasProperties) {
          ...
          final JsonArray propertyJson =
              JsonParser.parseString(rawPropertyJson).getAsJsonArray();
          for (JsonElement element : propertyJson) {
            ...
            if (BungeeCordProxy.isBungeeGuardEnabled() &&
                nameString.equals("bungeeguard-token")) {
              if (foundBungeeGuardToken ||
                  !BungeeCordProxy.isValidBungeeGuardToken(valueString)) {
                bungeeDisconnect(socketConnection);
                return;
              }
            ...
            properties.add(new GameProfile.Property(nameString, valueString,
                                                    signatureString));
          }

          if (BungeeCordProxy.isBungeeGuardEnabled() &&
              !foundBungeeGuardToken) {
            bungeeDisconnect(socketConnection);
            return;
          }
        }
        ...
        final GameProfile gameProfile =
            new GameProfile(playerUuid, "test", properties);
  ...
  private static void bungeeDisconnect(@NotNull PlayerConnection connection) {
    LOGGER.warn(
        "{} tried to log in without valid BungeeGuard forwarding information.",
